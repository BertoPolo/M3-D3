...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("https://api.pexels.com/v1/search?query=" + "forest", {
    ...
    .then((body) => {
      ...
      let filteredArtists = body.photos.filter((pic) => pic.photographer.includes("Brandon") || pic.photographer.includes("Felix"));
      console.log("FILTERED ARTISTS", filteredArtists);

      //EX13
      let reducedIds = body.photos.reduce((acc, curr) => acc + parseInt(curr.id), 0);
